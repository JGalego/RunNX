...
fn test_graph_validation() {
    ...
    let invalid_node = runnx::graph::Node::new(
        "invalid".to_string(),
        "Relu".to_string(),
    ...
    for i in 0..4 {
        ...
        let handle = thread::spawn(move || {
            ...
            inputs.insert(
                "input".to_string(),
                Tensor::from_array(
                    Array2::from_shape_vec((1, 3), vec![i as f32, (i + 1) as f32, (i + 2) as f32])
                        .unwrap()
                        .into_dyn(),
                ),
            );
