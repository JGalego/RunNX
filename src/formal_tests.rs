...
mod formal_verification_tests {
    ...
    proptest! {
        ...
        fn test_add_commutativity(
            a in prop::array::uniform32(prop::num::f32::NORMAL),
            b in prop::array::uniform32(prop::num::f32::NORMAL)
        ) {
            let shape = [2, a.len() / 2];
        ...
        fn test_softmax_probability_distribution(
            ...
            if let Ok(tensor) = Tensor::from_shape_vec(&[data.len()], data) {
                if let Ok(softmax_result) = tensor.softmax() {
                    ...
                    // Finite-precision probabilities may round to the endpoints.
                    for &value in softmax_result.data().iter() {
                        prop_assert!((0.0..=1.0).contains(&value), "Softmax value out of range: {}", value);
                    }
                }
            }
        }
    ...
    proptest! {
        ...
        fn test_softmax_numerical_stability(
            data in prop::collection::vec(-10.0f32..10.0, 1..10),
            ...
            if dim1 * dim2 <= data.len() {
                ...
                if let Ok(tensor) = Tensor::from_shape_vec(&shape, data[..dim1*dim2].to_vec()) {
                    ...
                    if softmax_result.is_ok() {
                        ...
                        for row in result.data().outer_iter() {
                    ...
                    if let Ok(t) = Tensor::from_shape_vec(&[dim1 + 1, dim2], vec![0.0; (dim1 + 1) * dim2]) {
                        let concat_result = tensor.concat_with_contracts(&t, 1);
                        prop_assert!(concat_result.is_err(), "Concat should fail with incompatible shapes");
                    }

                    // Test slice contracts with invalid parameters
                    let slice_result = tensor.slice_with_contracts(&[0], &[1], None, Some(&[0]));
                    prop_assert!(slice_result.is_err(), "Slice should fail with a zero step");
    ...
    proptest! {
        ...
        fn test_matmul_associativity(
            a_data in prop::collection::vec(-10.0f32..10.0, 4..16),
            b_data in prop::collection::vec(-10.0f32..10.0, 4..16),
            c_data in prop::collection::vec(-10.0f32..10.0, 4..16)
        ) {
            ...
            if a_data.len() >= dim * dim && b_data.len() >= dim * dim && c_data.len() >= dim * dim {
                ...
                for (l, r) in left.data().iter().zip(right.data().iter()) {
                    let tolerance = 1e-4 * l.abs().max(r.abs()).max(1.0);
                    prop_assert!((l - r).abs() <= tolerance);
                }
            }
        }
    }
    ...
    proptest! {
        ...
        fn test_relu_properties(
            ...
            let element_count = (data.len() / 2) * 2;
            let shape = [element_count / 2, 2];
            let tensor = Tensor::from_array(Array2::from_shape_vec(shape, data[..element_count].to_vec()).unwrap());
