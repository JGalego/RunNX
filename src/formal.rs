...
impl contracts::AdditionContracts for Tensor {
    fn add_with_contracts(&self, other: &Tensor) -> Result<Tensor> {
        ...
        debug_assert_eq!(result.shape(), self.shape(), "Result shape must match input shape");
...
impl contracts::MatMulContracts for Tensor {
    fn matmul_with_contracts(&self, other: &Tensor) -> Result<Tensor> {
        ...
        if self.shape()[1] != other.shape()[0] {
            return Err(crate::error::OnnxError::invalid_dimensions(
                format!("Inner dimensions must match: {} vs {}", 
...
pub mod why3_integration {
    ...
    pub fn to_why3_tensor(tensor: &Tensor) -> String {
        format!(
            "{{ shape = {}; data = {}; valid = true }}",
            format!("[{}]", tensor.shape().iter()
                .map(|x| x.to_string())
                .collect::<Vec<_>>()
                .join("; ")),
            format!("[{}]", tensor.data().iter()
                .map(|x| x.to_string())
                .collect::<Vec<_>>()
                .join("; "))
        )
...
mod tests {
    ...
    use super::contracts::{AdditionContracts, ActivationContracts, StabilityContracts};
