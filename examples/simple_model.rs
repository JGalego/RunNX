...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input_tensor = Tensor::from_array(Array2::from_shape_vec((1, 3), input_data).unwrap().into_dyn());
    ...
    for (i, (&expected, &actual)) in expected_result.iter().zip(actual_data.iter()).enumerate() {
        ...
        if diff > 1e-6 {
            println!("❌ Mismatch at index {}: expected {:.6}, got {:.6} (diff: {:.6})", 
    ...
    let results_match = actual_data.iter().zip(loaded_data.iter())
    ...
    let b = Tensor::from_array(Array2::from_shape_vec((2, 3), vec![1., 1., 1., 1., 1., 1.])?);
